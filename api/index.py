...
def translate_text(text, language):
    ...
    try:
        translator = EasyGoogleTranslate(
            source_language="ko", target_language=language.split("-")[0], timeout=10
        )
...
def parallel_translate_texts(texts, language):
    with ThreadPoolExecutor() as executor:
        translated_texts = list(
            executor.map(lambda text: translate_text(text, language), texts)
        )
...
def login_url(country_code):
    ...
    session.get(
        "https://auth.riotgames.com/.well-known/openid-configuration", headers=headers2
    )
...
def get_login_token(session, sdk_sid, country_code):
    ...
    response = session.get(
        "https://authenticate.riotgames.com/api/v1/login", headers=check_headers
    )
...
def get_access_token(login_token):
    ...
    response1 = session.post(
        "https://auth.riotgames.com/api/v1/login-token", headers=headers1, json=data1
    )
    ...
    response2 = session.post(
        "https://auth.riotgames.com/api/v1/authorization", headers=headers2, json=data2
    )
...
def fetch_token():
    ...
    if not token_data:
        ...
        return jsonify({"error": "Token expired", "new_url": new_url["login_url"]})
